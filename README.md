...
# AI Shopify Assistant: Virtual Shopkeeper

## Claude Integration (MCP Webhook)

- **/api/mcp**: MCP-compatible webhook endpoint for Claude (Anthropic LLM) to call during conversations. Handles tool_use calls for product queries, cart management, checkout, etc.
- **/api/mcp/manifest**: Returns a manifest describing the available tools for Claude to call. Update this as you add more capabilities.
